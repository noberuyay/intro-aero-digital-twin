...
describe("trim-response physics", () => {
  test("9.1 numerical case matches the reference calculation", () => {
    ...
    const trimAngleRad =
      calculateTrimAngleRad(0.04, -0.8);
    ...
    const product =
      calculateDisturbanceProduct(-0.8, 2.0);
  ...
  test("9.2 behavioral case changes to a positive slope and destabilizing tendency", () => {
    ...
    expect(response.cm).toBeCloseTo(
      0.08991642,
      8
    );
  ...
  test("zero disturbance produces a neutral tendency", () => {
    ...
    expect(
      calculateDisturbanceProduct(-0.8, 0)
    ).toBe(0);

    expect(
      classifyDisturbance(-0.8, 0)
    ).toBe("neutral");
  });
